...
Every one of these files is kept, and two properties govern that. There is no configuration document behind them either — the names and their defaults are `static final` constants, declared in pairs:

{% snip ice:core/src/main/java/org/apache/iceberg/TableProperties.java#L322-L341 | the retention and metrics defaults, as constants %}

`write.metadata.previous-versions-max` bounds how many entries the `metadata-log` array inside the current file retains — `METADATA_PREVIOUS_VERSIONS_MAX_DEFAULT = 100`. `write.metadata.delete-after-commit.enabled` decides whether the files that fall off the end are actually deleted, and `METADATA_DELETE_AFTER_COMMIT_ENABLED_DEFAULT` is **`false`**. A long-lived table therefore accumulates one `*.metadata.json` per commit indefinitely by default, most of them no longer listed anywhere.

The other three constants in that block belong to later chapters and are worth knowing where to find: `METADATA_COMPRESSION_DEFAULT` is `"none"`, which is why the `.gz` extension above is an opt-in rather than the norm; `METRICS_MAX_INFERRED_COLUMN_DEFAULTS_DEFAULT` is `100` and `DEFAULT_WRITE_METRICS_MODE_DEFAULT` is `"truncate(16)"`, the two numbers Chapter 2.4 §6 and Chapter 4.3 both reason from. **`TableProperties` is the only statement of these defaults in the tree** — `docs/docs/configuration.md` tabulates them, but the constants are what the code reads.
