...
The **opt-in flags**, on the api interfaces, which only set booleans: `OverwriteFiles.validateNoConflictingData()`, `validateNoConflictingDeletes()`, `validateAddedFilesMatchOverwriteFilter()`; `RowDelta.validateNoConflictingDataFiles()`, `validateNoConflictingDeleteFiles()`, `validateDeletedFiles()`; `ReplacePartitions.validateNoConflictingData()`, `validateNoConflictingDeletes()`, `validateAppendOnly()`. Then three that take an argument instead of flipping a switch: `validateFromSnapshot(long)` on all three, `conflictDetectionFilter(Expression)` on `RowDelta` and `OverwriteFiles` only, and `RowDelta.validateDataFilesExist(Iterable)` — which is not a flag at all. It appends to `referencedDataFiles`, and `BaseRowDelta.validate` runs the check whenever that collection is non-empty. `ReplacePartitions` has no conflict-detection filter, and does not need one: the partitions it is replacing *are* the predicate, which is why `BaseReplacePartitions.validate` hands the family a `PartitionSet` where the other two hand it an `Expression`.

The mapping from switch to check is not one-to-one, and `BaseRowDelta.validate` is where that is easiest to see. `validateNoConflictingDeleteFiles()` sets a single boolean that drives **two** checks: `validateNoNewDeletesForDataFiles`, itself nested behind `!removedDataFiles.isEmpty()`, and then `validateNoNewDeleteFiles`. `validateDeletedFiles()` also does double duty — it calls `failMissingDeletePaths()`, which is absent from the table above because it is a precondition rather than a history walk, and its *negation* is handed to `validateDataFilesExist` as `skipDeletes`. Reading the flag names as a menu of checks will mislead; the method body is the mapping.
